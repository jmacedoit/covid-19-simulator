...
/**
 * MainStatsGraphic component.
 */

class MainStatsGraphic extends Component {

  render() {
    const { parametersChanges, stats } = this.props;
    const graphicData = [{
      data: stats.map(({ day, recovered }) => ({
        x: day,
        y: recovered
      })),
      id: 'Recovered'
    }, {
      data: stats.map(({ day, infected }) => ({
        x: day,
        y: infected
      })).slice(),
      id: 'Infected'
      ...
      data: stats.map(({ day, totalInfected }) => ({
        x: day,
        y: totalInfected
      })),
      id: 'All'
...
/**
 * Export MainStatsGraphic.
 */

export default MainStatsGraphic;
